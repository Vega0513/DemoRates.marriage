  ...
  if(cal_freq == TRUE){
    ...
    if (!is.na(method)){
      ...
      if (method=="Poisson"){
        ...
        # never married & not cohabiting to married
        mrg1.male.freq.est <- merge(agelist, freq.poi(male, nlm, nhm, 1, 1, nWeight, mfp), all.x=TRUE)
        mrg1.female.freq.est <- merge(agelist, freq.poi(female, nlm, nhm, 1, 1, nWeight, mfp), all.x=TRUE)
        
        # never married & cohabiting to married
        nmCm.male.freq.est <- merge(agelist, freq.poi(male, nlm, nhm, 5, 11, nWeight, mfp), all.x=TRUE)
        nmCm.female.freq.est <- merge(agelist, freq.poi(female, nlm, nhm, 5, 11, nWeight, mfp), all.x=TRUE)
        
        # Divorced
        divorced.male.freq.est <- merge(agelist, freq.poi(male, nlm, 95, 2, 2, nWeight, mfp), all.x=TRUE)
        divorced.female.freq.est <- merge(agelist, freq.poi(female, nlm, 95, 2, 2, nWeight, mfp), all.x=TRUE)
        
        # Remarriage of Widowed & not cohabiting
        rw.male.freq.est <- merge(agelist, freq.poi(male, nlm, 95, 3, 3, nWeight, mfp), all.x=TRUE)
        rw.female.freq.est <- merge(agelist, freq.poi(female, nlm, 95, 3, 3, nWeight, mfp), all.x=TRUE)
        
        # remarriage of widowed & cohabiting
        rwC.male.freq.est <- merge(agelist, freq.poi(male, nlm, 95, 7, 12, nWeight, mfp), all.x=TRUE)
        rwC.female.freq.est <- merge(agelist, freq.poi(female, nlm, 95, 7, 12, nWeight, mfp), all.x=TRUE)
        
        # Remarriage of Divorced & not cohabiting
        rd.male.freq.est <- merge(agelist, freq.poi(male, nlm, 95, 4, 4, nWeight, mfp), all.x=TRUE)
        rd.female.freq.est <- merge(agelist, freq.poi(female, nlm, 95, 4, 4, nWeight, mfp), all.x=TRUE)
        
        # remarriage of divorced & cohabiting
        rdC.male.freq.est <- merge(agelist, freq.poi(male, nlm, 95, 8, 13, nWeight, mfp), all.x=TRUE)
        rdC.female.freq.est <- merge(agelist, freq.poi(female, nlm, 95, 8, 13, nWeight, mfp), all.x=TRUE)
        
        # never married to cohabiting
        nmtoC.male.freq.est <- merge(agelist, freq.poi(male, nlm, 95, 1, 5, nWeight, mfp), all.x=TRUE)
        nmtoC.female.freq.est <- merge(agelist,freq.poi(female, nlm, 95, 1, 5, nWeight, mfp), all.x=TRUE)
        
        # widowed to cohabiting
        wtoC.male.freq.est <- merge(agelist, freq.poi(male, nlm, 95, 3, 6, nWeight, mfp), all.x=TRUE)
        wtoC.female.freq.est <- merge(agelist, freq.poi(female, nlm, 95, 3, 6, nWeight, mfp), all.x=TRUE)
        
        # divorced to cohabiting
        dtoC.male.freq.est <- merge(agelist, freq.poi(male, nlm, 95, 4, 7, nWeight, mfp), all.x=TRUE)
        dtoC.female.freq.est <- merge(agelist, freq.poi(female, nlm, 95, 4, 7, nWeight, mfp), all.x=TRUE)
        
        # cohabiting to never married
        Ctonm.male.freq.est <- merge(agelist, freq.poi(male, nlm, 95, 5, 8, nWeight, mfp), all.x=TRUE)
        Ctonm.female.freq.est <- merge(agelist, freq.poi(female, nlm, 95, 5, 8, nWeight, mfp), all.x=TRUE)
        
        # cohabiting to widowed
        Ctow.male.freq.est <- merge(agelist, freq.poi(female, nlm, 95, 7, 9, nWeight, mfp), all.x=TRUE)
        Ctow.female.freq.est <- merge(agelist, freq.poi(female, nlm, 95, 7, 9, nWeight, mfp), all.x=TRUE)
        
        # cohabiting to divorced
        Ctod.male.freq.est <- merge(agelist, freq.poi(female, nlm, 95, 8, 10, nWeight, mfp), all.x=TRUE)
        Ctod.female.freq.est <- merge(agelist, freq.poi(female, nlm, 95, 8, 10, nWeight, mfp), all.x=TRUE)
